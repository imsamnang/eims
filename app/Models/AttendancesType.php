...
class AttendancesType extends Model
{
    public static $path = [
    ...
    public static function getData($id = null, $edit = null, $paginate = null, $search = null)
    {
        $pages['form'] = array(
            'action'  => array(
                'add'    => url(Users::role() . '/study/' . AttendancesType::$path['url'] . '/add/'),
            ),
        );
        ...
        $get = AttendancesType::orderBy('id', $orderBy);
            ...
            foreach ($get as $key => $row) {
                $data[$key]         = array(
                    ...
                    'action'        => [
                        ...
                        'delete' => url(Users::role() . '/study/' . AttendancesType::$path['url'] . '/delete/' . $row['id']),
                    ]
                );
                ...
                if (request('search.value')) {
                    foreach (request('columns') as $i => $value) {
                        if ($value['searchable']) {
                            ...
                            } elseif ($value['data'] == 'credit_absent') {
                                $query->orWhere('credit_absent', 'LIKE', '%' . request('search.value') . '%');
                            }
                        }
                    }
                }
    ...
    {
        $response           = array();
        $validator          = Validator::make(request()->all(), FormAttendancesType::rulesField(), FormAttendancesType::customMessages(), FormAttendancesType::attributeField());
        if ($validator->fails()) {
            $response       = array(
                'success'   => false,
            ...
            try {
                ...
                $add = AttendancesType::insertGetId($values);

                if ($add) {
                    if (request()->hasFile('image')) {
                        $image      = request()->file('image');
                        AttendancesType::updateImageToTable($add, ImageHelper::uploadImage($image, AttendancesType::$path['image']));
                    } else {
                        ImageHelper::uploadImage(false, AttendancesType::$path['image'], AttendancesType::$path['image'], public_path('/assets/img/icons/image.jpg'), null, true);
                    }

                    $response       = array(
                        'success'   => true,
                        'type'      => 'add',
                        'data'      => AttendancesType::getData($add),
                        'message'   => array(
    ...
    public static function updateToTable($id)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $update = AttendancesType::where('id', $id)->update($values);
                if ($update) {
                    if (request()->hasFile('image')) {
                        $image      = request()->file('image');
                        AttendancesType::updateImageToTable($id, ImageHelper::uploadImage($image, AttendancesType::$path['image']));
                    }
                    $response       = array(
                        'success'   => true,
                        'type'      => 'update',
                        'data'      => AttendancesType::getData($id),
                        'message'   => array(
                            'title' => Translator::phrase('success'),
    ...
    public static function updateImageToTable($id, $image)
    {
        ...
        if ($image) {
            try {
                $update =  AttendancesType::where('id', $id)->update([
                    'image'    => $image,
                ]);
    ...
    public static function deleteFromTable($id)
    {
        if ($id) {
            $id  = explode(',', $id);
            if (AttendancesType::whereIn('id', $id)->get()->toArray()) {
                if (request()->method() === 'POST') {
                    try {
                        $delete    = AttendancesType::whereIn('id', $id)->delete();
                        if ($delete) {
                            $response       =  array(
                                'success'   => true,
                                'message'   => array(
