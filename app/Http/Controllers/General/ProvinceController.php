...
class ProvinceController extends Controller
{
    ...
    public function index($param1 = 'list', $param2 = null, $param3 = null)
    {
        ...
        if ($param1 == 'list') {
            if (strtolower(request()->server('CONTENT_TYPE')) == 'application/json') {
                return Provinces::getData(null, null, 10);
            } else {
                $data = $this->list($data);
            }
            ...
            $data['title']    = Users::role(app()->getLocale()) . ' | ' . __('Add Province');
        } elseif ($param1 == 'edit') {
            if (request()->method() === 'POST') {
                return Provinces::updateToTable($id);
            }
            $data = $this->show($data, $id, $param1);
            $data['title']    = Users::role(app()->getLocale()) . ' | ' . __('Edit Province');
        } elseif ($param1 == 'view') {
            $data = $this->show($data, $id, $param1);
            $data['title']    = Users::role(app()->getLocale()) . ' | ' . __('View Province');
        } elseif ($param1 == 'delete') {
            return Provinces::deleteFromTable($id);
    ...
    public function list($data)
    {
        ...
        $response = $table->get()->map(function ($row) {
            ...
            $row['image'] = ImageHelper::site(Provinces::$path, $row['image']);
            $row['action']  = [
    ...
    public function show($data, $id, $type)
    {
        ...
        if ($id) {
            ...
            $response           = Provinces::whereIn('id', explode(',', $id))->get()->map(function ($row) {
                $row['image'] = $row['image'] ? ImageHelper::site(Provinces::$path, $row['image']) : ImageHelper::prefix();
                $row['action']  = [
                    'edit'   => url(Users::role() . '/' . 'general/' . Provinces::$path['url'] . '/edit/' . $row['id']),
                    'view'   => url(Users::role() . '/' . 'general/' . Provinces::$path['url'] . '/view/' . $row['id']),
                    'delete' => url(Users::role() . '/' . 'general/' . Provinces::$path['url'] . '/delete/' . $row['id']),
