...
use App\Http\Requests\FormNationality;
...
class NationalityController extends Controller
{
    ...
    public function index($param1 = 'list', $param2 = null, $param3 = null)
    {
        ...
        if ($param1 == 'list') {
            if (strtolower(request()->server('CONTENT_TYPE')) == 'application/json') {
                return Nationality::getData(null, null, 10);
            } else {
                $data = $this->list($data);
            }
        } elseif (strtolower($param1) == 'list-datatable') {
            if (strtolower(request()->server('CONTENT_TYPE')) == 'application/json') {
                return  Nationality::getDataTable();
            } else {
                $data = $this->list($data);
            }
        } elseif ($param1 == 'add') {
            if (request()->ajax()) {
                if (request()->method() === 'POST') {
                    return Nationality::addToTable();
                }
            }
            $data = $this->show($data, null, $param1);
            $data['title']    = Users::role(app()->getLocale()) . ' | ' . __('Add Nationality');
        } elseif ($param1 == 'edit') {
            if (request()->method() === 'POST') {
                return Nationality::updateToTable($id);
            }
            $data = $this->show($data, $id, $param1);
            $data['title']    = Users::role(app()->getLocale()) . ' | ' . __('Edit Nationality');
        } elseif ($param1 == 'view') {
            $data = $this->show($data, $id, $param1);
            $data['title']    = Users::role(app()->getLocale()) . ' | ' . __('View Nationality');
        } elseif ($param1 == 'delete') {
            return Nationality::deleteFromTable($id);
    ...
    public function list($data)
    {
        ...
        $response = $table->get()->map(function ($row) {
            ...
            $row['image'] = ImageHelper::site(Nationality::$path, $row['image']);
            $row['action']  = [
    ...
    public function show($data, $id, $type)
    {
        ...
        if ($id) {
            $response           = Nationality::whereIn('id', explode(',', $id))->get()->map(function ($row) {
                $row['image'] = $row['image'] ? ImageHelper::site(Nationality::$path, $row['image']) : ImageHelper::prefix();
                $row['action']  = [
                    'edit'   => url(Users::role() . '/' . 'general/' . Nationality::$path['url'] . '/edit/' . $row['id']),
                    'view'   => url(Users::role() . '/' . 'general/' . Nationality::$path['url'] . '/view/' . $row['id']),
                    'delete' => url(Users::role() . '/' . 'general/' . Nationality::$path['url'] . '/delete/' . $row['id']),
