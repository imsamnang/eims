...
class StudyModalitiesController extends Controller
{
    ...
    public function index($param1 = 'list', $param2 = null, $param3 = null)
    {
        $breadcrumb  = [
            [
                'title' => __('Study'),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . StudyModality::path('url')),
            ],
            [
                'title' => __('List Study Modality'),
                'status' => false,
        ...
        $data['formName'] = 'study/' . StudyModality::path('url');
        $data['formAction'] = '/add';
        $data['listData']       = array();
        ...
        if ($param1 == null || $param1 == 'list') {
            ...
            $breadcrumb[]  = [
                'title' => __($param1),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . StudyModality::path('url') . '/' . $param1),
            ];
            if (request()->method() === 'POST') {
                return StudyModality::addToTable();
            }
            ...
            $breadcrumb[]  = [
                'title' => __($param1),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . StudyModality::path('url') . '/' . $param1 . '/' . $id),
            ];
            if (request()->method() === 'POST') {
                return StudyModality::updateToTable($id);
            }
            ...
            $data = $this->show($data, $id, $param1);
            $data['view']  = StudyModality::path('view') . '.includes.view.index';
        } elseif ($param1 == 'delete') {
            if (request()->method() === 'POST') {
                return StudyModality::deleteFromTable($id);
            }
            abort(404);
        ...
        $data['instituteFilter']['data']           = Institute::whereIn('id', StudyModality::groupBy('institute_id')->pluck('institute_id'))
    ...
    public function show($data, $id, $type)
    {
        ...
        $data['title'] = Users::role(app()->getLocale()) . ' | ' . __('Study Modality') . ' | ' . __($type);
        return $data;
    }
    ...
    public function report()
    {
        ...
        config()->set('app.title', __('List Study Modality'));
        config()->set('pages.parent', StudyModality::path('view'));
        ...
        config()->set('pages.title', __('List Study Modality'));
        return view(StudyModality::path('view') . '.includes.report.index', $data);
    }
}
