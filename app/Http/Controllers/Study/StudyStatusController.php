    ...
    public function index($param1 = 'list', $param2 = null, $param3 = null)
    {
        ...
        if ($param1 == 'list') {
            if (strtolower(request()->server('CONTENT_TYPE')) == 'application/json') {
                return StudyStatus::getData(null, null, 10,request('search'));
            } else {
                $data = $this->list($data);
            }
        } elseif (strtolower($param1) == 'list-datatable') {
            if (strtolower(request()->server('CONTENT_TYPE')) == 'application/json') {
                return StudyStatus::getDataTable();
            } else {
                $data = $this->list($data);
            }
        } elseif ($param1 == 'add') {
        ...
        $pages['form']['validate'] = [
            'rules'       =>  FormStudyStatus::rulesField(),
            'attributes'  =>  FormStudyStatus::attributeField(),
            'messages'    =>  FormStudyStatus::customMessages(),
            'questions'   =>  FormStudyStatus::questionField(),
        ];
