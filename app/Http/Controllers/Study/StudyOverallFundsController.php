...
class StudyOverallFundsController extends Controller
{
    ...
    public function index($param1 = 'list', $param2 = null, $param3 = null)
    {
        $breadcrumb  = [
            [
                'title' => __('Study'),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . StudyOverallFund::path('url')),
            ],
            [
                'title' => __('List Study Overall Fund'),
                'status' => false,
        ...
        if ($param1 == null || $param1 == 'list') {
            ...
            $breadcrumb[]  = [
                'title' => __($param1),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . StudyOverallFund::path('url') . '/' . $param1),
            ];
            ...
            $breadcrumb[]  = [
                'title' => __($param1),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . StudyOverallFund::path('url') . '/' . $param1 . '/' . $id),
            ];
            if (request()->method() === 'POST') {
            ...
            $data = $this->show($data, $id, $param1);
            $data['view']  = StudyOverallFund::path('view') . '.includes.view.index';
        } elseif ($param1 == 'delete') {
            if (request()->method() === 'POST') {
                return StudyOverallFund::deleteFromTable($id);
            }
            abort(404);
        ...
        $data['instituteFilter']['data']           = Institute::whereIn('id', StudyOverallFund::groupBy('institute_id')->pluck('institute_id'))
    ...
    {
        $table = StudyOverallFund::orderBy('id', 'DESC');
        ...
        $response = $table->get()->map(function ($row, $nid) use ($count) {
            ...
            $row->image = ImageHelper::site(StudyOverallFund::path('image'), $row->image);
            $row->action  = [
        ...
        $data['title']    = Users::role(app()->getLocale()) . ' | ' . __('List StudyOverallFund');
        return $data;
    }
    ...
    public function show($data, $id, $type)
    {
        ...
        if ($id) {
            ...
            $response =  $table->get()->map(function ($row) {
                $row['image'] = $row['image'] ? ImageHelper::site(StudyOverallFund::path('image'), $row['image']) : ImageHelper::prefix();
                $row['action']  = [
                    'edit'   => url(Users::role() . '/' . 'study/' . StudyOverallFund::path('url') . '/edit/' . $row['id']),
                    'view'   => url(Users::role() . '/' . 'study/' . StudyOverallFund::path('url') . '/view/' . $row['id']),
                    'delete' => url(Users::role() . '/' . 'study/' . StudyOverallFund::path('url') . '/delete/' . $row['id']),
        ...
        $data['title'] = Users::role(app()->getLocale()) . ' | ' . __('StudyOverallFund') . ' | ' . __($type);
        return $data;
    }
    ...
    public function report()
    {
        ...
        config()->set('app.title', __('List StudyOverallFund'));
        config()->set('pages.parent', StudyOverallFund::path('view'));
        ...
        config()->set('pages.title', __('List StudyOverallFund'));
        return view(StudyOverallFund::path('view') . '.includes.report.index', $data);
    }
}
