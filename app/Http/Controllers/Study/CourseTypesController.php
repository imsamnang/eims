...
class CourseTypesController extends Controller
{
    ...
    public function index($param1 = 'list', $param2 = null, $param3 = null)
    {
        $breadcrumb  = [
            [
                'title' => __('Study'),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . CourseTypes::path('url')),
            ],
            [
                'title' => __('List Course Type'),
                'status' => false,
        ...
        if ($param1 == null || $param1 == 'list') {
            ...
            $breadcrumb[]  = [
                'title' => __($param1),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . CourseTypes::path('url') . '/' . $param1),
            ];
            ...
            $breadcrumb[]  = [
                'title' => __($param1),
                'status' => 'active',
                'link'  => url(Users::role() . '/study/' . CourseTypes::path('url') . '/' . $param1 . '/' . $id),
            ];
            if (request()->method() === 'POST') {
            ...
            $data = $this->show($data, $id, $param1);
            $data['view']  = CourseTypes::path('view') . '.includes.view.index';
        ...
        $data['instituteFilter']['data']           = Institute::whereIn('id', CourseTypes::groupBy('institute_id')->pluck('institute_id'))
    ...
    {
        $table = CourseTypes::orderBy('id', 'DESC');
        ...
        $data['title']    = Users::role(app()->getLocale()) . ' | ' . __('List Course Type');
        return $data;
    }
    ...
    public function show($data, $id, $type)
    {
        ...
        if ($id) {
            ...
            $response =  $table->get()->map(function ($row) {
                $row['image'] = $row['image'] ? ImageHelper::site(CourseTypes::path('image'), $row['image']) : ImageHelper::prefix();
                $row['action']  = [
                    'edit'   => url(Users::role() . '/' . 'study/' . CourseTypes::path('url') . '/edit/' . $row['id']),
                    'view'   => url(Users::role() . '/' . 'study/' . CourseTypes::path('url') . '/view/' . $row['id']),
                    'delete' => url(Users::role() . '/' . 'study/' . CourseTypes::path('url') . '/delete/' . $row['id']),
        ...
        $data['title'] = Users::role(app()->getLocale()) . ' | ' . __('Course Type') . ' | ' . __($type);
        return $data;
    }
    ...
    public function report()
    {
        ...
        config()->set('app.title', __('List Course Type'));
        config()->set('pages.parent', CourseTypes::path('view'));
        ...
        config()->set('pages.title', __('List Course Type'));
        return view(CourseTypes::path('view') . '.includes.report.index', $data);
    }
}
