...
<td data-toggle="tooltip" rel="tooltip" data-placement="left" class="{{ $setClass  }}" title="{{ $setTitle }}">
    ...
    @if(array_key_exists($i ,$holiday))
    <div class="rotate"
        style="writing-mode: vertical-rl; text-orientation: mixed; transform: rotate(-180deg);margin: auto;">
        <p style="margin: 0rem;">
            {{$setText}}
        </p>
    </div>
    ...
    @if($routine['success'])
    <table>
        @foreach ($routine['data'][0]['children'] as $k => $section)
            <tr>
                {{-- <td>
                    <div class="d-flex">
                        <span>{{$section["times"]["start_time"]}}</span>
                        &#9866;
                        <span>{{$section["times"]["end_time"]}}</span>
                    </div>
                </td> --}}
            <td>sec-{{$k + 1}}</td>
                <td class="px-4">0</td>
