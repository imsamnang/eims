...
<script>
    $(document).on("change",".check-reference",function(event){
            ...
            var role = $(this).val();
            if(role == 6 || role == 7 ){
                $('#reference-student').removeClass("d-none").find("select").prop("disabled",false);
            }else if(role != 1 && role != 9){
                $('#reference-staff').removeClass("d-none").find("select").prop("disabled",false);
            }
        });
</script>
@endsection
