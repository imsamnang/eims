...
var sweetalert2Locale = {
    __: function ($keyword) {
        if (window.sweetalert2_i18n[$keyword]) {
            return window.sweetalert2_i18n[$keyword];
        }
        return $keyword;
    }
};
        ...
        t.length && ((e = t),
            (a = new Chart(e, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: "Performance",
                        data: [0, 20, 10, 30, 15, 40, 20, 60, 60,[0, 20, 10, 30, 15, 40, 20, 60, 60]]
                    }]
                }
            })),
            e.data("chart", a));
...
var DatatableBasic = (function () {
    ...
    if (typeof $.fn.dataTable !== "undefined") {
        ...
        e.find("tbody>tr").dblclick(function () {
            $(this).find('[data-toggle="table-checked"]').prop("checked", true);
...
var ModalsTriggerSubmit = (function () {
    var e = $(".modal");
    e.length && e.each(function () {
        var $self = $(this);
        $self.find("#btn-submit").unbind().click(function () {
            $self.find("form:first").submit();
        });

        if ($self.is("#modal-add")) {
            ...
            $self.on("hidden.bs.modal", function () {
                ...
                $self.find("form:first").get(0).reset();
...
function FormQuizAnswers(forms) {
    ...
    e.length && e.each(function () {
        ...
        if ($(this).data("validation")) {
            $(this).unbind().attr({
                ...
                onSubmit: (event, validation,load) => {
                    event.preventDefault();
...
FormLesson();
FormLesson($("form#form-subjects"));



/*
 *   <form data-console="#console" data-show-errors="#show-errors" action="{url}">
 *   @param forms $("form")
 */
var channel = typeof pusher != "undefined" ? pusher.subscribe("import-channel") : null;

function FormExecute(forms) {
    var e = forms ? forms : $('form[id="form-execute"]');
    var aj = null;
    e.length && e.each(function () {
        var cons = $(this).data("console");
        var showErrors = $(this).data("show-errors");

        channel.bind(cons, ({
            data
        }) => {
            var container = $("<div>").addClass("border table-responsive p-2");
            if (data.data.length) {
                var table = $("<table>").addClass("table table-sm table-xs text-info").appendTo(container);
                $.each(data.data, function () {
                    table.append('<td>' + this + '</td>');
                });
                $(cons + ">code").append(container);
            }

            if (data.success) {

                if (data.total) {
                    $(cons + ">code").append('<div class="text-green">ភ្ជាប់ទៅម៉ាស៊ីនមេបានជោគជ័យ</div>');
                    $(cons + ">code").append('<div class="text-white">ទិន្នន័យសរុប : ' + data.total + '</div>');

                } else {
                    container.append('<div class="text-white text-sm">ជួរដេកទី : ' + data.row + '</div>');
                    container.append('<div class="text-green text-sm">បញ្ចូលបានជោគជ័យ</div>');
                }


            } else if (data.hasOwnProperty("errors") && Object.values(data.errors).length) {

                $.each(Object.values(data.errors), function () {
                    container.append('<div class="text-warning text-sm">' + this + '</div>');
                });
                container.append('<div class="text-white text-sm">ជួរដេកទី : ' + data.row + '</div>');
                container.append('<div class="text-danger text-sm">បញ្ចូលមិនបានជោគជ័យ</div>');

                $(showErrors).append(container.clone());

            }
            $(cons).animate({
                scrollTop: $(cons).prop("scrollHeight")
            }, "fast");
        });
        $(this).on("submit", function (event) {
            ...
            $(cons + ">code").html("");
            $(showErrors).html("");
            var file = $(this).find('[name="file"]').get(0).files[0];
            if (file) {
                $(cons + ">code").append('<div class="text-text">បញ្ចូលឯកសារឈ្មោះ​ ' + file.name + '</div>');
                $(cons).animate({
                    scrollTop: $(cons).prop("scrollHeight")
                }, "fast");

                var formData = new FormData(this);
                formData.append("console", cons);
                if (aj) {
                    aj.abort();
                }
                aj = $.ajax({
                    ...
                    data: formData,
                    beforeSend: function () {
                        $(cons + ">code").append('<div class="text-text">ភ្ជាប់ទៅម៉ាស៊ីនមេ...</div>');
                    },
                    success: function (response) {
                        if (response.success) {
                            $(cons + ">code").append('<div class="text-center p-4"><div class="text-blue">' + response.message + '</div></div>');
                            $(cons).animate({
                                scrollTop: $(cons).prop("scrollHeight")
                            }, "fast");
                        } else if (response.message) {
                            $(cons + ">code").append('<div class="text-danger">' + response.message + '</div>');
                            $(cons).animate({
                                scrollTop: $(cons).prop("scrollHeight")
                            }, "fast");
                        }
                ...
                $(cons + ">code").append('<div class="text-text">សូមបញ្ចូលឯកសារ</div>');
                $(cons).animate({
                    scrollTop: $(cons).prop("scrollHeight")
                }, "fast");
            }
                ...
                return {
                    results: $.map(response.data, function (obj) {
                        return {
                            ...
                            image: $.inArray("image", Object.keys(obj)) != -1 ? obj.image : ($.inArray("photo", Object.keys(obj)) != -1 ? obj.photo : obj.profile)
                        };
                    }),
                    pagination: {
...
var ChangeText = (function () {
    ...
    CardMaker = function () {
        ...
        e.length && e.each(function (i, element) {
            ...
            card[i] = new Card({
                container: $(this),
                form: $("form#form-card-make-" + i),
                layout: $(this).data("layout"),
                frame_foreground: $(this).data("foreground"),
                                ...
                                return $.ajax({
                                    ...
                                    success: response => {
                                        if (response.success) {
                                            ...
                                            $('[data-toggle="card-maker"]').each((i) => {
                                                card[i].set({
                                                    frame_foreground: response.data.foreground,
...
var CertificateMaker = function () {
    ...
    e.length && e.each(function (i, element) {
        $(this).attr("id", $(this).attr("id") + "-" + i);
        certificate[i] = new Certificate({
            container: $(this),
            form: $("form#form-certificate-make-" + i),
            layout: $(this).data("layout"),
            frame_foreground: $(this).data("foreground"),
            frame_background: $(this).data("background"),
            data: $(this).data("user")
                            ...
                            return $.ajax({
                                ...
                                success: response => {
                                    if (response.success) {
                                        ...
                                        $('[data-toggle="certificate-maker"]').each((i) => {
                                            certificate[i].set({
                                                frame_foreground: response.data.foreground,
                                                frame_background: response.data.background,
                                                layout: response.data.layout
                                            });
                                            certificate[i].update();
                                        });
...
var Dropdown = (function () {
    ...
    Croppie = (function () {
        ...
        e.length && e.each(function () {
            ...
            var cr = $(self).croppie({
                ...
                viewport: {
                    width: $(self).data("viewport-width") ? $(self).data("viewport-width") : 250,
                    height: $(self).data("viewport-height") ? $(self).data("viewport-height") : 300,
                    type: $(self).data("type") ? $(self).data("type") : "square"
                },
                boundary: {
                    width: "100%",
                    height: "250px"
                ...
                update: function (data) {
                    cr.croppie("result", {
                        ...
                        var target = $(self).attr("data-update");
            ...
            $(self).parents("form").unbind("submit").on("submit", function (event) {
                event.preventDefault();
    ...
    QrCode = (function () {
        ...
        e.length && e.each(function () {
            ...
            $(this).parents("form").find('#submit-one').click((event) => {
                event.preventDefault();
                $(this).parents("form").attr('action', $(this).parents("form").attr('action-one'));
                $(this).parents("form").submit();
            });
            $(this).parents("form").find('#submit-all').click((event) => {
                event.preventDefault();
                $(this).parents("form").attr('action', $(this).parents("form").attr('action-all'));
                $(this).parents("form").submit();
            });
        });
    })(),
    ...
    QrCodeReader = (function () {
        ...
        function scan() {
            a.length && a.each(function () {
                ...
                self.find("#btn-open").click(function () {
                    start();
                    self.find("#preview").removeClass("d-none");
                });
                self.find("#btn-close").click(function () {
                    scanner.stop();
                    $(this).prop("disabled", true);
                    self.find("#btn-open").prop("disabled", false);
                    self.find("#preview").addClass("d-none");
                });
                ...
                function start() {
                    Instascan.Camera.getCameras().then(function (cameras) {
                        if (cameras.length > 0) {
                            ...
                            self.find("#btn-close").prop("disabled", false);
                            self.find("#btn-open").prop("disabled", true);
                        } else {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $.each(response.data, function (i, u) {
                                ...
                                if ($(target).length) {
                                    template = `
                                            <li class="list-group-item px-0" data-uid="${u.id}">
                                                <div class="row align-items-center">
                                                    ...
                                                    <div class="col ml--2">
                                                        <h4 class="mb-0">
                                                            <a href="${u.action.view}">${u.name}</a>
                                                        </h4>
                                                        <small>${u.study_course_session.name}</small>
                                                    </div>
                                                </div>
                                            </li>
                                        `;
                                } else {
                                    template = `<div data-notify="container" data-id="${u.id}" class="border-0 list-group-item list-group-item-action col-xs-11 col-sm-3 alert bg-{0}" role="alert">
                                            ...
                                            <div class="row align-items-left px-3">
                                                ...
                                                <div class="col ml--2">
                                                    ...
                                                    <p class="text-sm mb-0"> ${u.study_course_session.name}</p>
                                                    <p class="text-sm mb-0"> ${response.message}</p>
                                                </div>
                                            </div>
                ...
                $(this).addClass("sweet-alert-loaded").on("click", function (event) {
                    ...
                    switch ($(this).data("sweet-alert")) {
                        case "confirm":
                            ...
                            var fixedid = '';
                            $.each(id.split(","), function (i, value) {
                                /**
                                 * eq(1) = td id
                                 */
                                fixedid += $('table#datatable-basic').find('tr[data-id="' + value + '"] td').eq(1).text() + ',';
                            });



                            swal({
                                title: sweetalert2Locale.__('Are you sure?'),
                                html: sweetalert2Locale.__('You wont be able to revert this!') + '<br> ID : ' + fixedid,
                                type: "warning",
                                showCancelButton: !0,
                                buttonsStyling: !1,
                                confirmButtonClass: "btn btn-danger",
                                confirmButtonText: sweetalert2Locale.__('Yes delete!'),
                                cancelButtonClass: "btn btn-secondary",
                                cancelButtonText: sweetalert2Locale.__('Cancel')
                            }).then(e => {
                                if (e.value) {
                                    ...
                                    ajaxDelete = $.ajax({
                                        ...
                                        beforeSend: function () {
                                            Swal({
                                                title: sweetalert2Locale.__('Deleting'),
                                                showCloseButton: true,
                                                allowOutsideClick: false,
                                                onBeforeOpen: () => {
                                                    Swal.showLoading();
                                        ...
                                        success: function (data) {
                                            if (data.success) {
                                                if (id) {
                                                    ...
                                                    $.each(id.split(","), (i, value) => {
                                                        table.api().row(`[data-id="${value}"]`).remove().draw();
                                                    });
                                                }

                                                swal({
                                                    title: sweetalert2Locale.__('Deleted'),
        ...
        e.length && e.each(function () {
            $(this).unbind().on("click", function (event) {
                ...
                if ($('[data-toggle="datatable-ajax"]').length) {
                    $('[data-toggle="datatable-ajax"]').each(function () {
                        var t = $(this).DataTable();
                        t.ajax.reload(function (callback) {
                            if (callback) {
                                Swal.close();
                            }
                        });
...
var ReadPDF = (function () {
    ...
    e.length &
        e.on("change", function (event) {
            ...
            if ($(view).length) {
                $(view).html('<iframe src="' + URL.createObjectURL(pdf) + '" frameborder="0" scrolling="no" class="w-100" height="300"></iframe>');
            }
            if ($(target).length) {
                $(target).after(canvas);
...
/** Table Filter jquery-ui */

var TableFilter = (function () {
    var e = $('[data-toggle="table-filter"]');
    e.length &&
        e.each(function () {
