...
var DatatableAjax = (function () {
    ...
    if (typeof $.fn.dataTable !== "undefined") {
        ...
        $.each(ajaxTableData.getThead(), (i, th) => {
            ...
            if (th.type == "checkbox") {
                ...
                column.render = function (data, type, row) {
                    ...
                    if (th.link && indexSplitSign(row, th.link)) {
                        ct = '<a href="' + indexSplitSign(row, th.link) + '" target="_blank">' + ct + '</a>';
                    }
                    return ct;
                };
                ...
                column.render = function (data, type, row) {
                    ...
                    return '<a href="' + indexSplitSign(row, th.link) + '">' + ct.substring(0, ct.lastIndexOf(th.join)) + '</a>';
                }
            } else if (th.type == "modal" && th.link) {
                column.render = function (data, type, row) {
                    return '<a href="' + indexSplitSign(row, th.link) + '" data-href="' + indexSplitSign(row, th.link) + '" data-checked-show="view" data-target="#modal" data-backdrop="static" data-keyboard="false">' + indexSplitSign(row, th.key) + '</a>';
                }

            } else if (th.type == "text") {
                column.render = function (data, type, row) {
                    var text = th.key.split(",");
                    var ct = "";
                    $.each(text, (i, o) => {
                        if (o) {
                            ct += indexSplitSign(row, o) + th.join;
                    ...
                    return ct.substring(0, ct.lastIndexOf(th.join));
                };
        ...
        var t = e.DataTable({
            ...
            ajax: {
                ...
                error: function (xhr, error, code) {
                    if (($.inArray(code, ["abort", "Not Found"]) == -1)) {
                        t.ajax.reload();
                    }
            ...
            fnDrawCallback: function () {
                ...
                $('.dataTables_paginate > .pagination').addClass("loaded");
        ...
        var form_filter = $('form#form-datatable-filter');
        ...
        form_filter.length &&
            form_filter.each(function () {
                $(this).on('submit', function (event) {
                    ...
                    var new_action = $(this).serializeArray().reduce(function (obj, item) {
                        urlhelper.set({}, updateQueryStringParameter(action, item.name, item.value));
                        return updateQueryStringParameter(action.replace("list", "list-datatable"), item.name, item.value);
                    }, "");

                    if (new_action) {
                        t.ajax.url(new_action).load();
                    } else {
                        urlhelper.set({}, action);
                        t.ajax.url(action + "-datatable").load();
                    }
